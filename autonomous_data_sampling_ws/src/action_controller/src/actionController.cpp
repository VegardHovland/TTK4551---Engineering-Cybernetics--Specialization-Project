				...
				has_active_goal_(false)
{
		...
		trajectory_pub = node_.advertise<trajectory_msgs::MultiDOFJointTrajectory>("/cmd_3dnav", 10);
	...
	void executeTrajectory(){
		if(toExecute.joint_names[0]=="virtual_joint" && toExecute.points.size()>0){
			for(int k=0; k<toExecute.points.size(); k++){
				...
				//desired_wp.points[k].transforms[0].x = point.translation.x;
  				//desired_wp.points[k].transforms[0].y = point.translation.y;
  				//desired_wp.points[k].transforms[0].z = point.translation.z;
